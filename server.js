...
function restorePlayer(socket, roomId, player, replacementToken = null) {
  const room = rooms[roomId];
  player.id = socket.id;
  if (replacementToken) player.token = replacementToken;
...
function startTurnTimer(roomId) {
  ...
  clearTurnTimer(room);
  if (!room.turnDurationSeconds) return;
...
function drawTileForCurrentPlayer(roomId, reason = 'draw') {
  const room = rooms[roomId];
  if (!room || room.players.length === 0) return;
  ...
  io.to(roomId).emit('turnNotice', {
    message: reason === 'timeout'
      ? (drewTile
        ? `${player.name} se quedó sin tiempo, tomó una ficha y pasó el turno.`
        : `${player.name} se quedó sin tiempo y pasó el turno.`)
      : `${player.name} tomó una ficha del pozo.`
  });
  advanceTurn(roomId);
}
...
function handleTurnTimeout(roomId) {
  const room = rooms[roomId];
  if (!room || !room.started) return;
  drawTileForCurrentPlayer(roomId, 'timeout');
}

io.on('connection', (socket) => {

  socket.on('createRoom', ({ playerName, playerToken, turnTimeEnabled, turnMinutes, turnSeconds, turnDurationSeconds }) => {
    const roomId = uuidv4().slice(0, 6).toUpperCase();
    const requestedSeconds = turnDurationSeconds ?? ((Number(turnMinutes) || 0) * 60 + (Number(turnSeconds) || 0));
    rooms[roomId] = {
      players: [],
      pool: createTiles(),
      board: [],
      ...
      started: false,
      lastBoardState: [],
      turnDurationSeconds: turnTimeEnabled ? normalizeTurnSeconds(requestedSeconds) : 0,
      turnEndsAt: null,
      turnTimer: null
    };
  ...
  socket.on('joinRoom', ({ roomId, playerName, playerToken }) => {
    ...
    const disconnectedByName = room.players.find(p => {
      return p.connected === false && normalizePlayerName(p.name) === normalizePlayerName(playerName);
    });
    if (disconnectedByName) return restorePlayer(socket, roomId, disconnectedByName, playerToken || uuidv4());
  ...
  socket.on('playTurn', ({ roomId, board, usedTileIds }) => {
    ...
    if (!player.opened) {
      ...
      const openingValue = initialMeldValue(room.board, board, player.hand, playedTileIds);
      if (openingValue < 30) {
        return socket.emit('turnError', `Tu primera bajada debe sumar 30 o mas. Esta suma ${openingValue}.`);
      }
    }
  ...
  socket.on('drawTile', ({ roomId }) => {
    ...
    drawTileForCurrentPlayer(roomId, 'draw');
  });
...
function getRoomPublicState(roomId) {
  ...
  return {
    ...
    turnDurationSeconds: room.turnDurationSeconds || 0,
